...
class Total_data() :
    def __init__(self, debug):
        ...
        self.Average_volumn_threshold = self.msg.Average_volumn_threshold
                ...
                while retry < 4 :
                    try :
                        ...
                        try :
                            ...
                            retry += 1
                            if retry < 4 :
                                logging.info(f"[ERROR] Total_data :: downloadingdata :: Connect error {file_name} on {retry} attempt!!")
                                logging.info("Wait 10 second for retry!!")
                                time.sleep(10)
                            else :
                                logging.info(f"[ERROR] Total_data :: downloadingdata :: Connect error {file_name} reach limit times!!")
                        ...
                        #if (sum(data["Volume"][-5:])/5) > self.Average_volumn_threshold :
                        if stock_type == "nasdaqlisted" :
                            subpath = "stock\\"
                        elif stock_type == "otherlisted" :
                            subpath = "etf\\"
                        else :
                            logging.info("ERROR : Subpath not found!!")
                            break
                        ...
                        if os.path.isfile(file_path + file_name):
                            ...
                            if status == -1 :
                                logging.info(f"[ERROR] Total_data :: downloadingdata :: Getting old data {file_name} Failure. ")
                            ...
                            logging.info(f"{file_name}  update {diff_data.shape[0]} line")
                        else:
                            data.to_csv(file_path + file_name, index = False)
                            logging.info(f"{file_name} write。")
